...
const TweetEntityTransform : Transformer<IPostDocument, ITweet> = 
    (post) => new Tweet(
        post.id,
        post.tweet.text,
...
const transformMentions : Transformer<ITweetMentionDocument, ITweetMention> = 
    (mention) => new TweetMention(
        mention.screenName,
        mention.id
    )

const transformMedia : Transformer<ITweetMediaDocument, IMedia> = 
    (media) => isImage(media.type) ? 
